...
Then("the user should  be naviaged to the homepage", async function () {
  const logoutLocator = page.locator(loginPage.logoutButton);
  // const [innerText] = await logoutLocator.allInnerTexts();
...
Then(
  "the user should be given invalid credentials {string}",
  async function (errorMessage) {
    const messageLocator = page.locator(loginPage.message);
    const [innerText] = await messageLocator.allInnerTexts();
    await expect(messageLocator).toBeVisible();
    console.log("The user entered to the Loginpage");
    if (innerText.trim() !== errorMessage) {
